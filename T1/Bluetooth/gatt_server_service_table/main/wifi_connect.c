...
#define ESP_WIFI_SCAN_AUTH_MODE_THRESHOLD WIFI_AUTH_OPENs
...
static const char *WIFI_CONNECT = "wifi station";
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < 20) {
            esp_wifi_connect();
            s_retry_num++;
            ESP_LOGI(WIFI_CONNECT, "retry to connect to the AP");
        } else {
            xEventGroupSetBits(s_wifi_event_group, WIFI_FAIL_BIT);
        }
        ESP_LOGI(WIFI_CONNECT,"connect to the AP fail");
...
//     ESP_LOGI(WIFI_CONNECT, "ESP_WIFI_MODE_STA");
//     wifi_init_sta();
// }
